<template>
  <div class="chase-game-wrapper">
    ...
    <!-- 打字核心战场 (Typing Arena) -->
    <div class="typing-cockpit" @click="focusHiddenInput">
      <!-- 隐藏原生输入框捕获物理按键与输入法上屏 -->
      <input
        ref="hiddenInputRef"
        type="text"
...
const {
  ...
  gameCharsList,
  onSuccess: (step = 1) => onCharSuccess(step),
  onError: () => onCharError(),
  onKeystroke: () => { totalKeystrokes.value++; }
});
...
const nextExpectedKey = computed(() => {
  ...
  const currentLen = displayInput.value.length;
